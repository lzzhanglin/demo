...
(function (global, factory) {
    ...
    function isValid(m) {
        if (m._isValid == null) {
            ...
            var isNowValid = !isNaN(m._d.getTime()) &&
                flags.overflow < 0 && !flags.empty && !flags.invalidMonth && !flags.invalidWeekday && !flags.weekdayMismatch && !
    ...
    function unescapeFormat(s) {
        return regexEscape(s.replace('\\', '').replace(/\\(\[)|\\(\])|\[([^\]\[]*)\]|\\(.)/g, function (matched, p1, p2,
                                                                                                        p3, p4) {
            return p1 || p2 || p3 || p4;
        }));
    }
    ...
    function toISOString(keepOffset) {
        ...
        if (isFunction(Date.prototype.toISOString)) {
            ...
            if (utc) {
                ...
                return new Date(this.valueOf() + this.utcOffset() * 60 * 1000).toISOString().replace('Z', formatMoment(
                    m, 'Z'));
            }
        }
